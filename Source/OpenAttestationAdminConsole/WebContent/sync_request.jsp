...
<script language="javascript">
	function attest(){
		for(var i=0;i<document.syncAttest.host.length;i++){
	           if(document.syncAttest.host[i].checked==true){
	        	   document.syncAttest.host[i].value = document.syncAttest.host_name[i].value;
	           }
	       }
		document.syncAttest.submit();
	}
	
	function selectAll(){
		if (document.syncAttest.all_host.checked == false){
			for(var i=0;i<document.syncAttest.host.length;i++){
				document.syncAttest.host[i].checked = false;
			}
		}else{
			for(var i=0;i<document.syncAttest.host.length;i++){
				document.syncAttest.host[i].checked = true;
			}
		}
	}
</script>
...
<td valign="top">
   <form name="syncAttest" method="post" action="SyncAttestServlet">
   <table class="main_div">
     <tr><td class="nav_font">Attestation Request - Sync Mode</td></tr>
     ...
     <tr><td align="center">
	     ...
	     <tr>
	     	<td width="200px" class="table_header_font" align="center">Time Threshold (millisecond)</td>
	     	<td align="center"><input type="text" name="timeThreshold"></td>
	     </tr>
